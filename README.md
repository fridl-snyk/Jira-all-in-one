...
Creates Jira tickets based on Snyk vulnerability database IDs and sets the title as the vulnerability title.
The body contains all issues in the project they came from that have the same vulnerability ID.
The vulnerabilities are initally sorted by a specific tag that is set on a Snyk project.  This does not grab projects without a tag, but that can easily be added.
